...
func (e *Executor) StartMiner() error {
	...
	execCmd := fmt.Sprintf("export PATH=$PATH:~/tools/filecoin/calibration; nohup  lotus-miner run >>%v 2>&1 &", name, e.conf.GetLogPath(name))
...
func (e Executor) StartWorker() error {
	...
	execCmd := fmt.Sprintf(
		"mkdir -p %v;export PATH=$PATH:~/tools/filecoin/calibration; nohup exec -a %v lotus-worker --worker-repo=%v run --listen=%v --addpiece=%v --precommit1=%v --precommit2=%v --commit=%v >>%v 2>&1 &",
